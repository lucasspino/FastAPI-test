...
crypt = CryptContext(schemes=["bcrypt"])  # El instalado en passlib
...
async def login(form: OAuth2PasswordRequestForm = Depends()):
    ...
    # Verificando si contraseña encriptada es correcta
    if not crypt.verify(form.password, user.password):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="La contraseña no es correcta")

    access_token = {"sub": user.username, "exp": datetime.now(
        timezone.utc) + timedelta(minutes=ACCESS_TOKEN_DURATION)}
